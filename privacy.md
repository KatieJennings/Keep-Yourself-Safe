...
1) **Punishment Data**: If any punishments are applied to you (warning, timeout, captcha, kick, ban), this data is logged for moderators of the server to view. This data is stored *forever*.
...
4) **Reputation Points**: Users can use the `+rep` and `-rep` commands on users. When this occurs, the reputation points will be logged and are viewable via the `$rep` command. This data is stored *forever*.
    - You cannot opt out of this feature.
5) **Malware Scans**: When users upload files, the bot will automatically scan them if they are executables, scripts or anything of that nature. This data is stored *forever*.
    - You cannot opt out of this feature.
6) **Messages**: When you send a message in a channel with the bot, it will store your message for a short period of time in memory only. A rolling history of about 10 messages is temporarily stored.
    - You can opt out of this feature by doing `/opt aspect: Generative AI option: Disabled` in any discord server with Keep Yourself Safe.
...
1) **Online Status**: Online status will be checked when users use the @there command/mention.
    - You can opt out of this feature by doing `/opt aspect: Online Status option: Disabled` in any discord server with Keep Yourself Safe
...
## Shared Data
We do not share any of the private data stored by the bot including messages and presence data. However, due to the nature of the Generative AI features, the data you send when mentioning the bot or being in the history of the reply may be used for training purposes by the model. Since the bot is free, we only use free-tier models, which usually train on the data sent. This wholly depends on the router and model used. 
